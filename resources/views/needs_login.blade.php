...
@section('content')
    ...
    <div class="profile-list">
        <form class="profile large loggedout" method="get" action="https://discord.com/api/v10/oauth2/authorize" onsubmit='if(Intl.DateTimeFormat().resolvedOptions().timeZone != undefined) document.getElementById("needslogin_state").value += ":"+Intl.DateTimeFormat().resolvedOptions().timeZone;'>
            <!--CSRF Token, redirect URL (and timezone) as `State`-->
            <input type="hidden" id="needslogin_state" name="state" value="{{ csrf_token() }}:{{ urlencode(str_replace("/_seamless", "", url()->current())) }}"/>
